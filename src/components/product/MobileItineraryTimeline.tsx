...
function DaySection({
    ...
    const dayLabel = (c?.dayLabel || `${dayIndex + 1}일차`).replace(/（.*?）/, '');
        ...
        return (
            <SpineRow>
                <img
                    ...
                    alt={`${productName} 일정${index + 1}｜몽골여행`}
                    loading="lazy"
                    decoding="async"
                    onError={hideBroken}
                    ...
                    {(slide.images || []).map((img, i) => (
                        <div key={i} className="relative shrink-0 w-[85%] snap-center">
                            <img
                                ...
                                alt={`${slide.title || productName} ${i + 1}｜몽골여행・몽골투어`}
                                loading="lazy"
                                decoding="async"
                                onError={hideBroken}
