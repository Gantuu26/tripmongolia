        ...
        <div className="flex h-full flex-col overflow-hidden rounded-2xl bg-[#FFF5F7] dark:bg-slate-900">
            ...
            <div className="flex-1 overflow-y-auto p-4">
                {activePage === 'overview' && <Frame>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3"><div><h4 className="mb-1 text-sm font-black text-[#E00B41]">포함 사항</h4><textarea value={settings.overview.includedText} onChange={e => onDocSection('overview', { includedText: e.target.value })} rows={5} className={`${fieldClass} resize-none text-[11px] font-semibold leading-relaxed text-slate-600`} placeholder="한 줄에 한 항목" /></div><div><h4 className="mb-1 text-sm font-black text-slate-500">불포함 사항</h4><textarea value={settings.overview.excludedText} onChange={e => onDocSection('overview', { excludedText: e.target.value })} rows={5} className={`${fieldClass} resize-none text-[11px] font-semibold leading-relaxed text-slate-600`} placeholder="한 줄에 한 항목" /></div></div><div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-xl border border-[#FFD9E0] bg-white p-3 text-center"><p className="text-[9px] font-black text-slate-400">참가 인원</p><p className="text-base font-black text-slate-700">{peopleCount}명</p></div><div className="rounded-xl bg-gradient-to-br from-[#E00B41] to-[#FF385C] p-3 text-center text-white"><p className="text-[9px] font-black">청구 금액(합계)</p><p className="text-xl font-black">{sampleTotal.toLocaleString()}원</p></div></div><div className="mt-2 grid grid-cols-2 gap-2"><div className="rounded-xl border border-[#FF385C] bg-[#FFF1F3] p-3 text-center"><p className="text-[9px] font-black text-[#E00B41]">예약금(신청 시 결제)</p><p className="text-lg font-black text-[#E00B41]">{sampleDeposit.toLocaleString()}원</p></div><div className="rounded-xl border border-[#FFD9E0] bg-white p-3 text-center"><p className="text-[9px] font-black text-slate-400">현지 결제 잔금</p><p className="text-lg font-black text-slate-700">{sampleLocal.toLocaleString()}원</p></div></div><div className="mt-4"><h4 className="mb-1 text-sm font-black text-[#E00B41]">인원별 요금 안내</h4><div className="overflow-hidden rounded-xl border border-[#FFD9E0]">{pricingTiers.map((tier, ti) => (<div key={ti} className={`grid grid-cols-[1.1fr_1.2fr_1fr_1fr_auto] items-center gap-1 border-b border-[#FFD9E0] p-1 last:border-b-0 ${tier.recommended ? 'bg-[#FFF1F3]' : ''}`}><input value={tier.people} onChange={e => updatePricingTier(ti, { people: e.target.value })} placeholder="인원" className={`${fieldClass} text-[10px] font-bold text-slate-700`} /><input value={tier.price} onChange={e => updatePricingTier(ti, { price: e.target.value })} placeholder="1인 가격" className={`${fieldClass} text-[10px] font-bold text-slate-900`} /><input value={tier.deposit} onChange={e => updatePricingTier(ti, { deposit: e.target.value })} placeholder="예약금" className={`${fieldClass} text-[10px] font-bold text-[#E00B41]`} /><input value={tier.balance} onChange={e => updatePricingTier(ti, { balance: e.target.value })} placeholder="잔금" className={`${fieldClass} text-[10px] font-bold text-slate-500`} /><div className="flex shrink-0 items-center"><button onClick={() => updatePricingTier(ti, { recommended: !tier.recommended })} title="추천"><span className={`material-symbols-outlined text-[15px] ${tier.recommended ? 'text-amber-500' : 'text-slate-300'}`}>star</span></button><button onClick={() => removePricingTier(ti)} title="행 삭제"><span className="material-symbols-outlined text-[15px] text-slate-300 hover:text-red-500">close</span></button></div></div>))}</div><button onClick={addPricingTier} className="mt-1 inline-flex items-center gap-0.5 text-[11px] font-black text-[#E00B41]"><span className="material-symbols-outlined text-[14px]">add</span>요금 행 추가</button><input value={settings.overview.pricingNote || ''} onChange={e => onDocSection('overview', { pricingNote: e.target.value })} placeholder="요금 안내 문구" className={`${fieldClass} mt-1 block w-full text-[10px] font-semibold text-slate-400`} /></div></div>
                </Frame>}
