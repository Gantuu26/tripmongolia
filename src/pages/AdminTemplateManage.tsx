...
export interface DayMeals { breakfast?: string; lunch?: string; dinner?: string; }
export interface TemplateDay { day: number; title: string; region?: string; activities: Activity[]; meals?: DayMeals; accommodation?: string; }
export interface PricingTier { people: string; price: string; deposit: string; balance: string; recommended?: boolean; }
...
export const defaultDocumentSettings = (): DocumentSettings => ({
    overview: {
        ...
        pricingTiers: [
            { people: '2인', price: '1,690,000', deposit: '507,000', balance: '1,183,000' },
            { people: '3~4인', price: '1,390,000', deposit: '417,000', balance: '973,000', recommended: true },
            { people: '5~6인', price: '1,250,000', deposit: '375,000', balance: '875,000' },
            { people: '7인 이상', price: '1,150,000', deposit: '345,000', balance: '805,000' },
        ],
        pricingNote: '예약금은 1인 가격의 30%이며, 예약 확정 시 입금됩니다. 잔금은 출발 30일 전까지 결제해 주세요.',
    },
    contract: {
...
export const mergeDocumentSettings = (value: any): DocumentSettings => {
    ...
    return {
        overview: { ...base.overview, ...(value.overview || {}), included: Array.isArray(value.overview?.included) ? value.overview.included : base.overview.included, pricingTiers: Array.isArray(value.overview?.pricingTiers) ? value.overview.pricingTiers : base.overview.pricingTiers },
...
const Frame: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <div className="mx-auto max-w-[960px] rounded-[22px] border border-[#FFD9E0] bg-white shadow-sm">{children}</div>
);
...
type TemplatePreviewProps = {
    ...
    onDayPatch?: (dayIdx: number, patch: Partial<TemplateDay>) => void;
...
export const TemplatePreview: React.FC<TemplatePreviewProps> = ({ name, description, days, documentSettings, customer, assignedGuide, dailyAccommodations, onNameChange, onDescriptionChange, onDocSection, onIncluded, onCancellation, onGuideNotice, onDayChange, onActivityChange, onAddDay, onAddActivity, onRemoveDay, onRemoveActivity, onDayActivitiesText, onDayPatch }) => {
    ...
    const pricingTiers = settings.overview.pricingTiers || [];
    const updatePricingTier = (i: number, patch: Partial<PricingTier>) => onDocSection('overview', { pricingTiers: pricingTiers.map((t, idx) => idx === i ? { ...t, ...patch } : t) });
    const addPricingTier = () => onDocSection('overview', { pricingTiers: [...pricingTiers, { people: '', price: '', deposit: '', balance: '' }] });
    const removePricingTier = (i: number) => onDocSection('overview', { pricingTiers: pricingTiers.filter((_, idx) => idx !== i) });
    // textarea 자동 높이 — 줄바꿈해도 위쪽 내용이 가려지지 않게 내용에 맞춰 늘어남
    const autoGrow = (el: HTMLTextAreaElement | null) => { if (!el) return; el.style.height = 'auto'; el.style.height = `${el.scrollHeight}px`; };
    // 관리자 결제 설정(/admin/payment)에 등록한 입금 계좌를 결제 안내(bankInfo)에 자동 표시.
    // 비어 있거나 기본 문구일 때만 채우고, 관리자가 직접 수정한 값은 덮어쓰지 않음.
    useEffect(() => {
        let cancelled = false;
        (async () => {
            try {
                const res: any = await (api as any).settings.get();
                const ba = normalizeBankAccount(res?.bank_account);
                if (cancelled || (!ba.bankName && !ba.accountNumber && !ba.accountHolder)) return;
                const text = `${[ba.bankName, ba.accountNumber].filter(Boolean).join(' ')}${ba.accountHolder ? `\n예금주: ${ba.accountHolder}` : ''}`;
                const cur = (settings.contract.bankInfo || '').trim();
                if (!cur || cur === defaultDocumentSettings().contract.bankInfo.trim()) {
                    onDocSection('contract', { bankInfo: text });
                }
            } catch { /* 설정 없으면 기존 값 유지 */ }
        })();
    ...
    const fieldClass = 'w-full rounded-md border border-transparent bg-transparent px-1 py-0.5 outline-none transition-colors hover:border-[#FFD9E0] hover:bg-white/80 focus:border-[#FF385C] focus:bg-white focus:ring-2 focus:ring-[#FF385C]/15';
    // Frame은 모듈 스코프로 이동 (입력마다 리마운트되어 스크롤·포커스가 튀던 문제 수정)

    return (
        <div className="flex h-full flex-col overflow-hidden rounded-2xl bg-[#FFF5F7] dark:bg-slate-900">
            <div className="border-b border-[#FFD9E0]/60 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
                <div className="mb-3 flex items-center justify-between gap-3">
                    ...
                    <span className="rounded-full bg-[#FF385C]/10 px-3 py-1 text-[11px] font-black text-[#E00B41]">Дарж засах</span>
                </div>
                <div className="grid grid-cols-4 gap-1.5 rounded-2xl bg-[#FFF1F3] p-1">
                    {pages.map(page => (
                        <button key={page.id} onClick={() => setActivePage(page.id)} className={`inline-flex min-w-0 items-center justify-center gap-1 rounded-xl px-2 py-2 text-[11px] font-black transition-colors ${activePage === page.id ? 'bg-white text-[#E00B41] shadow-sm' : 'text-slate-500 hover:bg-white/60'}`} title={page.label}>
            ...
            {customer ? (
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 border-b border-teal-200 bg-teal-50 px-4 py-2 text-[11px] font-bold text-[#E00B41] dark:border-teal-800 dark:bg-teal-900/20">
            ...
            <div className="flex-1 overflow-y-auto p-4">
                {activePage === 'overview' && <Frame>
                    <div className="flex items-center justify-between px-5 py-4">
                        <div className="flex items-center gap-2 text-[#E00B41]"><span className="material-symbols-outlined text-[28px]">landscape</span><div><p className="text-[12px] font-black">Trip Mongolia</p><p className="text-[8px] font-bold tracking-widest">몽골 현지 여행사</p></div></div>
                        <div className="text-right"><p className="text-[22px] font-black tracking-[0.12em] text-[#E00B41]">여행 일정표</p><input value={settings.overview.subtitle} onChange={e => onDocSection('overview', { subtitle: e.target.value })} className={`${fieldClass} max-w-[260px] text-right text-[10px] font-semibold leading-snug text-slate-400`} /></div>
                    </div>
                    <div className="relative h-[220px] overflow-hidden"><img src={mongoliaHero} alt="" className="h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-r from-[#B00935]/90 via-[#E00B41]/45 to-transparent" /><div className="absolute bottom-5 left-5 right-5 text-white"><span className="rounded-xl bg-[#E00B41] px-3 py-2 text-xs font-black">{tripLength}</span><input value={name} onChange={e => onNameChange(e.target.value)} placeholder="은하수·대자연 패키지" className={`${fieldClass} mt-3 block max-w-[520px] text-[28px] font-black leading-tight text-white placeholder:text-white/70`} /><input value={description || settings.overview.heroTagline} onChange={e => onDescriptionChange(e.target.value)} className={`${fieldClass} mt-1 max-w-[560px] text-xs font-semibold text-white/90`} /></div></div>
                    <div className="p-5"><h4 className="mb-2 flex items-center gap-1.5 text-sm font-black text-[#E00B41]"><span className="material-symbols-outlined text-base">check_circle</span>여행 개요</h4><textarea value={settings.overview.intro} onChange={e => onDocSection('overview', { intro: e.target.value })} rows={3} className={`${fieldClass} mb-3 resize-none text-[11px] font-semibold leading-relaxed text-slate-500`} /><div className="overflow-hidden rounded-xl border border-[#FFD9E0] text-xs">{[...rows, ['담당 가이드', guideText], ['숙박', accommodationSummary]].map(([label, value]) => <div key={label} className="grid grid-cols-[112px_1fr] border-b border-[#FFD9E0] last:border-b-0"><div className="bg-[#FFF5F7] px-3 py-2 font-black text-[#E00B41]">{label}</div><div className="px-3 py-2 font-semibold text-slate-700">{value}</div></div>)}</div>
                    <div className="mt-4 grid grid-cols-2 gap-3"><div><h4 className="mb-1 text-sm font-black text-[#E00B41]">포함 사항</h4><textarea ref={autoGrow} value={settings.overview.includedText} onChange={e => onDocSection('overview', { includedText: e.target.value })} onInput={e => autoGrow(e.currentTarget)} rows={4} style={{ overflow: 'hidden' }} className={`${fieldClass} resize-none text-[11px] font-semibold leading-relaxed text-slate-600`} placeholder="한 줄에 한 항목" /></div><div><h4 className="mb-1 text-sm font-black text-slate-500">불포함 사항</h4><textarea ref={autoGrow} value={settings.overview.excludedText} onChange={e => onDocSection('overview', { excludedText: e.target.value })} onInput={e => autoGrow(e.currentTarget)} rows={4} style={{ overflow: 'hidden' }} className={`${fieldClass} resize-none text-[11px] font-semibold leading-relaxed text-slate-600`} placeholder="한 줄에 한 항목" /></div></div><div className="mt-4"><h4 className="mb-1 text-sm font-black text-[#E00B41]">인원별 요금 안내</h4><div className="overflow-hidden rounded-xl border border-[#FFD9E0]"><div className="grid grid-cols-[1.1fr_1.2fr_1fr_1fr_auto] items-center gap-1 border-b border-[#FFD9E0] bg-[#FFF5F7] p-1 text-[9px] font-black uppercase tracking-wide text-slate-500"><span className="px-1.5">인원</span><span className="px-1.5">1인 기준 총가격</span><span className="px-1.5 text-[#E00B41]">예약금</span><span className="px-1.5">잔금</span><span className="w-[44px]" /></div>{pricingTiers.map((tier, ti) => (<div key={ti} className={`grid grid-cols-[1.1fr_1.2fr_1fr_1fr_auto] items-center gap-1 border-b border-[#FFD9E0] p-1 last:border-b-0 ${tier.recommended ? 'bg-[#FFF1F3]' : ''}`}><input value={tier.people} onChange={e => updatePricingTier(ti, { people: e.target.value })} placeholder="인원" className={`${fieldClass} text-[10px] font-bold text-slate-700`} /><input value={tier.price} onChange={e => updatePricingTier(ti, { price: e.target.value })} placeholder="1인 가격" className={`${fieldClass} text-[10px] font-bold text-slate-900`} /><input value={tier.deposit} onChange={e => updatePricingTier(ti, { deposit: e.target.value })} placeholder="예약금" className={`${fieldClass} text-[10px] font-bold text-[#E00B41]`} /><input value={tier.balance} onChange={e => updatePricingTier(ti, { balance: e.target.value })} placeholder="잔금" className={`${fieldClass} text-[10px] font-bold text-slate-500`} /><div className="flex shrink-0 items-center"><button onClick={() => updatePricingTier(ti, { recommended: !tier.recommended })} title="추천"><span className={`material-symbols-outlined text-[15px] ${tier.recommended ? 'text-amber-500' : 'text-slate-300'}`}>star</span></button><button onClick={() => removePricingTier(ti)} title="행 삭제"><span className="material-symbols-outlined text-[15px] text-slate-300 hover:text-red-500">close</span></button></div></div>))}</div><button onClick={addPricingTier} className="mt-1 inline-flex items-center gap-0.5 text-[11px] font-black text-[#E00B41]"><span className="material-symbols-outlined text-[14px]">add</span>요금 행 추가</button><input value={settings.overview.pricingNote || ''} onChange={e => onDocSection('overview', { pricingNote: e.target.value })} placeholder="요금 안내 문구" className={`${fieldClass} mt-1 block w-full text-[10px] font-semibold text-slate-400`} /></div></div>
                </Frame>}
                {activePage === 'contract' && <Frame><div className="p-5"><div className="mb-5 flex items-start justify-between"><div className="text-[#E00B41]"><p className="text-[12px] font-black">Trip Mongolia</p><p className="text-[8px] font-bold tracking-widest">몽골 현지 여행사</p></div><div className="rounded-lg bg-[#FF385C]/10 px-3 py-2 text-[9px] font-black text-[#E00B41]">계약일: 2026년 6월 4일</div></div><h3 className="text-center text-[30px] font-black tracking-[0.18em] text-[#E00B41]">여행 계약서</h3><p className="text-center text-xs font-semibold uppercase tracking-widest text-slate-500">Travel Contract</p><textarea value={settings.contract.intro} onChange={e => onDocSection('contract', { intro: e.target.value })} rows={3} className={`${fieldClass} mx-auto mt-4 block max-w-[520px] resize-none text-center text-[11px] font-semibold leading-relaxed text-slate-500`} /><div className="mt-5 overflow-hidden rounded-xl border border-[#FFD9E0] text-xs">{[['여행명', name || '은하수·대자연 패키지'], ['여행 기간', customer?.period || (totalDays ? tripLength : '일정표 참조')], ['여행 대금', customer?.totalAmount ? `${sampleTotal.toLocaleString()}원 (1인당 ${samplePrice.toLocaleString()}원)` : '인원별 요금표 참조'], ['가이드', guideText], ['숙박', accommodationSummary]].map(([label, value]) => <div key={label} className="grid grid-cols-[112px_1fr] border-b border-[#FFD9E0] last:border-b-0"><div className="bg-[#FFF5F7] px-3 py-2 font-black text-[#E00B41]">{label}</div><div className="px-3 py-2 font-semibold text-slate-700">{value}</div></div>)}</div><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-xl border border-[#FFD9E0] p-3"><p className="text-xs font-black text-[#E00B41]">취소 규정</p><div className="mt-2 space-y-1">{settings.contract.cancellationRows.slice(0, 5).map((row, idx) => <div key={idx} className="grid grid-cols-[1fr_90px] gap-1"><input value={row.period} onChange={e => onCancellation(idx, 'period', e.target.value)} className={`${fieldClass} text-[10px] font-semibold text-slate-500`} /><input value={row.fee} onChange={e => onCancellation(idx, 'fee', e.target.value)} className={`${fieldClass} text-[10px] font-semibold text-slate-500`} /></div>)}</div></div><div className="rounded-xl border border-[#FFD9E0] p-3"><p className="text-xs font-black text-[#E00B41]">결제 안내</p><input value={settings.contract.paymentMethod} onChange={e => onDocSection('contract', { paymentMethod: e.target.value })} className={`${fieldClass} mt-2 text-[10px] font-semibold text-slate-500`} /><input value={settings.contract.paymentDeadline} onChange={e => onDocSection('contract', { paymentDeadline: e.target.value })} className={`${fieldClass} mt-1 text-[10px] font-semibold text-slate-500`} /><textarea ref={autoGrow} value={settings.contract.bankInfo} onChange={e => onDocSection('contract', { bankInfo: e.target.value })} onInput={e => autoGrow(e.currentTarget)} rows={2} style={{ overflow: 'hidden' }} placeholder="입금 계좌·결제 안내(자유 입력)" className={`${fieldClass} mt-1 resize-none text-[10px] font-semibold leading-relaxed text-slate-500`} /><div className="mt-4 border-b border-slate-300 pb-1 text-[10px] text-slate-400">여행자 서명</div></div></div></div></Frame>}
                {activePage === 'detail' && <Frame><div className="p-5"><div className="mb-5 flex items-center justify-between"><input value={settings.detail.title} onChange={e => onDocSection('detail', { title: e.target.value })} className={`${fieldClass} text-[24px] font-black tracking-[0.12em] text-[#E00B41]`} /><span className="rounded-full bg-[#FF385C]/10 px-3 py-1 text-xs font-black text-[#E00B41]">{totalDays || 0}일간</span></div>
                    <div className="space-y-3">{days.map((day, dayIdx) => { const inText = textDays.has(dayIdx); return (<div key={dayIdx} className="grid grid-cols-[64px_1fr] gap-3">
                        <div className="flex flex-col items-center rounded-xl bg-gradient-to-b from-[#E00B41] to-[#FF385C] px-2 py-4 text-center text-white"><p className="text-[10px] font-black">DAY {day.day}</p><p className="mt-1 text-[11px] font-bold">{day.day}일차</p><button onClick={() => onRemoveDay(dayIdx)} className="mt-2 text-white/60 hover:text-white" title="Энэ өдрийг устгах"><span className="material-symbols-outlined text-[15px]">delete</span></button></div>
                        <div className="rounded-xl border border-[#FFD9E0] p-3">
                            <input value={day.title} onChange={e => onDayChange(dayIdx, 'title', e.target.value)} placeholder={`${day.day}일차 제목`} className={`${fieldClass} text-sm font-black text-[#E00B41]`} />
                            ...
                            {getAccommodation(day.day)?.name && <div className="mt-2 inline-flex items-center gap-1 rounded-full bg-teal-50 px-2.5 py-1 text-[10px] font-black text-[#E00B41]"><span className="material-symbols-outlined text-[13px]">hotel</span>숙박: {getAccommodation(day.day)?.name}</div>}
                            <div className="mt-2 flex items-center justify-between"><span className="text-[10px] font-bold text-slate-400">일정</span>{onDayActivitiesText && <button onClick={() => toggleTextDay(dayIdx)} className="text-[10px] font-bold text-[#E00B41] hover:text-[#0a7d6a]">{inText ? '↩ Нүд тус бүрээр засах' : '✎ Текстээр нэг дор'}</button>}</div>
                            {inText && onDayActivitiesText ? (
                                ...
                                <div className="relative mt-1 border-l-2 border-dashed border-[#FFD9E0] pl-4">{day.activities.map((activity, index) => <div key={index} className="relative pb-1.5 last:pb-0"><span className="absolute -left-[23px] top-2 h-3 w-3 rounded-full bg-[#E00B41] ring-2 ring-white" /><div className="flex items-center gap-1"><input value={activity.time || ''} onChange={e => onActivityChange(dayIdx, index, 'time', e.target.value)} placeholder="--:--" className={`${fieldClass} w-[58px] font-mono text-[11px] font-bold text-slate-400`} /><input value={activity.title} onChange={e => onActivityChange(dayIdx, index, 'title', e.target.value)} placeholder="항목명(예: 거북바위 관광)" className={`${fieldClass} flex-1 text-[11px] font-semibold text-slate-700`} /><button onClick={() => onRemoveActivity(dayIdx, index)} className="shrink-0 text-slate-300 hover:text-red-500" title="Устгах"><span className="material-symbols-outlined text-[15px]">close</span></button></div></div>)}
                                    <button onClick={() => onAddActivity(dayIdx)} className="mt-1 inline-flex items-center gap-0.5 text-[11px] font-black text-[#E00B41] hover:text-[#0a7d6a]"><span className="material-symbols-outlined text-[14px]">add</span>Зүйл нэмэх</button>
                                </div>
                            )}
                            <div className="mt-2 rounded-lg bg-[#FFF5F7] p-2">
                                <div className="mb-1 grid grid-cols-3 gap-1">
                                    <input value={day.meals?.breakfast || ''} onChange={e => onDayPatch?.(dayIdx, { meals: { ...day.meals, breakfast: e.target.value } })} placeholder="조식" className={`${fieldClass} text-[10px] font-semibold text-slate-600`} />
                                    <input value={day.meals?.lunch || ''} onChange={e => onDayPatch?.(dayIdx, { meals: { ...day.meals, lunch: e.target.value } })} placeholder="중식" className={`${fieldClass} text-[10px] font-semibold text-slate-600`} />
                                    <input value={day.meals?.dinner || ''} onChange={e => onDayPatch?.(dayIdx, { meals: { ...day.meals, dinner: e.target.value } })} placeholder="석식" className={`${fieldClass} text-[10px] font-semibold text-slate-600`} />
                                </div>
                                <input value={day.accommodation || ''} onChange={e => onDayPatch?.(dayIdx, { accommodation: e.target.value })} placeholder="숙박(예: Ugii Dalai Hotel)" className={`${fieldClass} text-[10px] font-semibold text-slate-600`} />
                            </div>
                        </div>
                    </div>); })}
                        {days.length === 0 && <p className="rounded-xl border border-dashed border-[#FFD9E0] py-6 text-center text-[11px] font-bold text-slate-400">доорх «Өдөр (DAY) нэмэх»-ээр хуваарь үүсгэж эхлээрэй.</p>}
                        <button onClick={onAddDay} className="w-full rounded-xl border-2 border-dashed border-[#FFD9E0] py-3 text-xs font-black text-[#E00B41] transition-colors hover:bg-[#FFF1F3]"><span className="material-symbols-outlined align-middle text-[16px]">add</span> Өдөр (DAY) нэмэх</button>
                    </div>
                ...
                {activePage === 'guide' && <Frame><div className="grid gap-4 p-5 sm:grid-cols-2"><div className="rounded-xl border border-[#FFD9E0] p-4"><h3 className="text-sm font-black text-[#E00B41]">안내·주의 사항</h3>{settings.guide.notices.slice(0, 5).map((item, idx) =><div key={idx} className="mt-3 flex gap-2"><span className="material-symbols-outlined text-[20px] text-[#E00B41]">info</span><div className="flex-1"><input value={item.title} onChange={e => onGuideNotice(idx, 'title', e.target.value)} className={`${fieldClass} text-xs font-black text-[#E00B41]`} /><textarea value={item.body} onChange={e => onGuideNotice(idx, 'body', e.target.value)} rows={2} className={`${fieldClass} mt-1 resize-none text-[10px] font-semibold leading-relaxed text-slate-500`} /></div></div>)}</div><div className="rounded-xl border border-[#FFD9E0] p-4"><h3 className="text-sm font-black text-[#E00B41]">여행 조건(요약)</h3><textarea value={settings.guide.conditions} onChange={e => onDocSection('guide', { conditions: e.target.value })} rows={4} className={`${fieldClass} mt-2 resize-none text-[10px] font-semibold leading-relaxed text-slate-500`} /><h3 className="mt-5 text-sm font-black text-[#E00B41]">여행 대금 결제</h3><textarea value={settings.guide.paymentInfo} onChange={e => onDocSection('guide', { paymentInfo: e.target.value })} rows={3} className={`${fieldClass} mt-2 resize-none text-[10px] font-semibold leading-relaxed text-slate-500`} /><h3 className="mt-5 text-sm font-black text-[#E00B41]">긴급 연락처</h3><input value={settings.guide.emergencyPhone} onChange={e => onDocSection('guide', { emergencyPhone: e.target.value })} className={`${fieldClass} mt-2 text-[10px] font-semibold text-slate-500`} /><input value={settings.guide.emergencyEmail} onChange={e => onDocSection('guide', { emergencyEmail: e.target.value })} className={`${fieldClass} text-[10px] font-semibold text-slate-500`} /></div></div><div className="relative h-[104px] overflow-hidden"><img src={mongoliaHero} alt="" className="h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-r from-white via-white/75 to-transparent" /><input value={settings.guide.closingMessage} onChange={e => onDocSection('guide', { closingMessage: e.target.value })} className={`${fieldClass} absolute left-5 top-7 max-w-[360px] text-sm font-black text-[#E00B41]`} /><div className="absolute bottom-0 left-0 right-0 flex items-center justify-between bg-[#E00B41] px-5 py-2 text-[10px] font-bold text-white"><span>Trip Mongolia</span><span>{settings.guide.emergencyEmail}</span></div></div></Frame>}
            </div>
        </div>
    );
};
                    ...
                    {templates.map(t => (
                        <div key={t.id} className="card" style={{ overflow: 'hidden' }}>
                            <div className="relative h-36 overflow-hidden">
                                ...
                                <div className="absolute inset-0 bg-gradient-to-r from-[#E00B41]/95 via-[#E00B41]/65 to-transparent" />
                                <div className="absolute left-5 top-4 text-white">
                                    <div className="flex items-center gap-2">
                                        <span className="material-symbols-outlined text-[24px]">landscape</span>
                                        <div>
                                            <p className="text-[11px] font-black">Trip Mongolia</p>
                                ...
                                <span className="absolute right-4 top-4 rounded-xl bg-white/90 px-3 py-1 text-xs font-black text-[#E00B41] shadow-sm">{Math.max(0, t.days.length - 1)} шөнө {t.days.length} өдөр</span>
                            </div>
                            ...
                            <div className="card-pad">
                                <div className="mb-4 grid grid-cols-4 gap-2">
                                    {[
                                        ...
                                        <div key={item.label} className="rounded-xl border border-[#FFD9E0]/80 bg-[#FFF5F7] px-2 py-2 text-center text-[#E00B41]">
                                    ...
                                    <div className="space-y-1">
                                        {t.days.slice(0, 3).map(d => (
                                            <div key={d.day} className="flex items-center gap-2 text-xs text-slate-500">
                                                <span className="flex h-7 w-9 flex-shrink-0 items-center justify-center rounded-lg bg-[#FF385C]/15 text-[11px] font-black text-[#E00B41]">{d.day} өдөр</span>
